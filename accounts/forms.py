...
↓↓←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←←↓↓
↓↓→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→→↓↓
↓↓      forms.py  Created by  Olorunshola Matins 2022                        ↓↓
...
class OnlinevoteUserCreationForm(UserCreationForm):
    class Meta(UserCreationForm):
        model = UserModel
        fields = ('email',)


class OnlinevoteUserChangeForm(UserChangeForm):
    class Meta:
        model = UserModel
        fields = ('email',)
...
class PasswordResetForm:
    ...
    def send_mail(self, context):
        ...
        email_plaintext_message = "{}change-password/{}/{}".format(settings.URL_FRONT,
                                                                   encoder(
                                                                       user_data.id),
                                                                   context['token'])
...
#         """
#         user_data = UserModel.objects.get(email=self.email)
#         email_plaintext_message = "{}email-reset/?uid={}/?token={}".format(settings.URL_FRONT,
#                                                                            encoder(
#                                                                                user_data.id),
#                                                                            context['token'])
#
#         first_name = UserModel.objects.get(email=self.email).first_name
#         link = email_plaintext_message
#
#         c = dict(
...
#             site_name="ARK"
#         )
#         html_message = render_to_string('account/email/email_reset.html', c)
#         plain_message = strip_tags(html_message)
#
#         subject, from_email, to = 'Email Reset for {title}'.format(title="Ark"), \
#                                   'noreply@somehost.local', UserModel.objects.get(email=self.email).email
#
#         msg = EmailMultiAlternatives(subject, plain_message, from_email, [to])
#
...
#
#         This allows subclasses to more easily customize the default policies
#         that prevent inactive users and users with unusable emails from
#         resetting their email.
#         """
#         email_field_name = UserModel.get_email_field_name()
#         active_users = UserModel._default_manager.filter(**{
#             '%s__iexact' % email_field_name: email,
